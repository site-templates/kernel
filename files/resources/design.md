...
**The accent appears at most three times on a page**: the rule that draws
across a leader on hover, a link's underline on hover, and the marker on the
Now page. It is never a button background, never a badge, never a heading.
...
| Class | Size | Use |
|---|---|---|
| `.display-hero` | `clamp(2.25rem, 5vw, 3.5rem)` / 1.08 / -0.021em | the homepage statement |
| `.display-page` | `clamp(1.875rem, 3.4vw, 2.5rem)` / 1.12 | an inner page title, a post title |
...
rather than laggy. On a phone the leader is dropped and the row stacks.
...
- A hover state with no `focus-visible` twin.
